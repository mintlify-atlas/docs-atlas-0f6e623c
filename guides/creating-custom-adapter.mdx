...
<Callout>
Learn more about the `adapter` here [here](/concepts/database#adapters).
</Callout>
</Step>
...
The `createSchema` method allows the [Better Auth CLI](/api-reference/cli/overview) to [generate](/api-reference/cli/overview/#generate) a schema for the database.
